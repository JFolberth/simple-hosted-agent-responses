...
Smoke-test coverage is provided by the external marketplace action `JFolberth/ai-smoketest@v1.0` — consumed directly from each deploy workflow rather than through a local composite wrapper. See [Smoke tests in CI](#smoke-tests-in-ci).
...
Each `update-agent*` job's **last step** invokes the [`JFolberth/ai-smoketest@v1.0`](https://github.com/marketplace/actions/ai-smoke-test) marketplace action against the freshly-deployed agent before the pipeline reports green. That produces **four invocations per pipeline run** — one per agent variant per IaC tool:
...
The marketplace action assumes the caller has already run `actions/checkout@v6` (so `tests_file` is on disk) and `azure/login@v3` (so the runner can call `az account get-access-token`). It does not perform either itself — this matches the existing composite-action contract used by `push-image`, `update-agent`, and `update-agent-source-code`.
